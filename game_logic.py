...
class MazeTowerDefenseGame:
    ...
    def spawn_enemies(self):
        ...
        #     self.enemies.append(enemy)
         # ---- CỐ ĐỊNH CHỈ SỐ THEO WAVE ----
        wave_multiplier = 1 #+ (self.current_wave * 0.2)
        num_enemies = 10 #+ int(self.current_wave * 4)

        base_health = int(50 * wave_multiplier)
        base_damage = int(self.current_wave * 2)
        
        base_speed = 1500 #30 + min(75, self.current_wave * 6)
        base_speed *= 1.0  # Không tăng tốc random nữa

        spawn_delay_base = 1.0 #max(0.5, 8.0 * (0.8 ** self.current_wave))  # Giữ nguyên
        # Không thêm bất kỳ random giảm delay nữa

        # Cố định tỉ lệ enemy đặc biệt
        tank_chance = min(0.35, 0.15 + self.current_wave * 0.03)
        fast_chance = min(0.45, 0.25 + self.current_wave * 0.04)
        ...
        print(f"Base Health: {base_health}, Base Speed: {base_speed}, Base Damage: {base_damage}")
        print(f"Tank chance: {tank_chance:.2%}, Fast chance: {fast_chance:.2%}")
        print(f"Spawn Delay: {spawn_delay_base:.1f}s")

        random.seed(0)  # Cố định seed để tạo kết quả như nhau mỗi lần chạy

        for i in range(num_enemies):
            # Định nghĩa enemy theo công thức có kiểm soát
            r = (i / num_enemies)  # Chia đều tỉ lệ
            if r < tank_chance:
                enemy_type = "tank"
            elif r < (tank_chance + fast_chance):
                enemy_type = "fast"
            ...
            type_data = dict(self.enemy_types[enemy_type])
            enemy_health = int(base_health * type_data['health_factor'])
            enemy_speed = base_speed * type_data['speed_factor']
            
            base_shot_damage = max(1, int(0.5 + self.current_wave * 0.1))
            enemy_damage = type_data['damage'] + base_damage

            if enemy_type in ["tank", "fast"]:
                enemy_health += int(self.current_wave * 10)
                enemy_damage += int(self.current_wave * 1.5)
                base_shot_damage = int(base_shot_damage * 1.1)

            type_data['damage'] = enemy_damage
            type_data['shoot_damage'] = base_shot_damage
